...
def transform_features(features):
    ...
    return pd.concat([aes_features, blowfish_features], ignore_index=True)


def svm_classifier_AES_vs_Blowfish(features):
    ...
    svm = SVC(C=0.5, gamma='scale', kernel='rbf', probability=True, random_state=RANDOM_SEED)
    ...
    print(f"Accuracy of AES vs Blowfish classifier: {accuracy * 100:.2f}%")
    ...
    model_path = os.path.join(MODELS_DIR, "svm_classifier_AES_v_Blowfish_optimized.joblib")
    dump(svm, model_path)
...
def main():
    ...
    svm_classifier_AES_vs_Blowfish(transformed_features)
